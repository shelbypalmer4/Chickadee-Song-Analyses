scatter3Dhack <- function (x, y = NULL, z = NULL, color = par("col"), pch = par("pch"), 
                           main = NULL, sub = NULL, xlim = NULL, ylim = NULL, zlim = NULL, 
                           xlab = NULL, ylab = NULL, zlab = NULL, scale.y = 1, angle = 40, 
                           axis = TRUE, tick.marks = TRUE, label.tick.marks = TRUE, 
                           x.ticklabs = NULL, y.ticklabs = NULL, z.ticklabs = NULL, 
                           y.margin.add = 0, y.axis.offset = 1, grid = TRUE, box = TRUE, 
                           lab = par("lab"), lab.z = mean(lab[1:2]), type = "p", highlight.3d = FALSE, 
                           # mar = c(5, 3, 4, 3) + 0.1, 
                           mar = c(1, 1, 1, 1) + 0.1,
                           bg = par("bg"), col.axis = par("col.axis"), 
                           ...
                           lty.hplot = par("lty"), log = "", asp = NA, ...) 
{
  ...
  if (type == "p" || type == "h") {
    ...
    if (highlight.3d && !(all(diff(daty) == 0))) 
      dat$col <- rgb(red = 0, 
                     green = seq(0, 1, length = n) * (y.range[2] - daty)/diff(y.range), 
                     blue = 0)
  } # hack: change to green
  p.lab <- par("lab")
  ...
  temp <- strwidth(paste0("M", format(rev(y.prty))[1]), cex = cex.axis * 
                     par("cex"), font = font.axis)
  lheight <- (strheight("\n") - strheight("M")) * asp
  lheight2 <- (strheight("\n") - strheight("M"))
  if (angle.2) 
      ...
      if (label.tick.marks) {
        ...
        text(i.y * yx.f + xx[1], i.y * yz.f + z.min, 
             y.ticklabs, pos = if (angle.1) 
               2
             else 4, offset = y.axis.offset, col = col.lab, 
...
# now hack MST 3D plotting function
plotMSThack <- function (tree, x = 1, y = 2, z = 3, col.pts = "darkred", col.segts = "darkred", 
          angle = 40, ...) 
{
  x = x
  y = y
  z = z
  g <- scatter3Dhack(tree[, x], tree[, y], tree[, 
                                                               z], color = col.pts, angle = angle, ...)
  from <- g$xyz.convert(tree[tree$from, x], tree[tree$from, 
                                                 y], tree[tree$from, z])
  to <- g$xyz.convert(tree[tree$to, x], tree[tree$to, y], tree[tree$to, 
                                                               z])
...
# hacking ggbiplot
ggbihack <- function (pcobj, choices = 1:2, scale = 1, pc.biplot = TRUE, 
                      ...
                      ...) 
{
  ...
  g <- ggplot(data = df.u, aes(x = xvar, y = yvar)) + xlab(u.axis.labs[1]) + 
    ylab(u.axis.labs[2]) + coord_equal() + theme_bw()
  if (var.axes) {
    if (circle) {
