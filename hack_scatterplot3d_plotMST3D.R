                           ...
                           lty.hplot = par("lty"), log = "", asp = NA, ...) 
{
  ...
  if (type == "p" || type == "h") {
    ...
    if (highlight.3d && !(all(diff(daty) == 0))) 
      dat$col <- rgb(red = 0, 
                     green = 0, 
                     blue = seq(0, 1, length = n) * (y.range[2] - daty)/diff(y.range),
                     alpha = 0.8)
  } # hack: change to blue, add transparency
  p.lab <- par("lab")
...
plotMSThack <- function (tree, x = 1, y = 2, z = 3, col.pts = "black", col.segts = "black", 
          angle = 40, ...) 
{
  x = x
  y = y
